"""
Phase 1 input: per-overpass TROPOMI NO2 (or CO, for D4) arrays over a ~100 km box around Pune.
...
Usage:
    python -m ecotrack.acquire.tropomi_cube                       # NO2 -> data/interim/tropomi/cube/
    python -m ecotrack.acquire.tropomi_cube --product co          # CO  -> data/interim/tropomi/cube_co/
...
# product -> (config section, output folder)
PRODUCTS = {"no2": ("tropomi", DATA_INTERIM / "tropomi" / "cube"),
            "co": ("tropomi_co", DATA_INTERIM / "tropomi" / "cube_co")}
CUBE_DIR = PRODUCTS["no2"][1]
FILL = -9999.0
...
def month_images(m_start, m_end, cfg, corridor, n_total, product="no2"):
    """QC'd images in the month whose corridor valid fraction passes the usability threshold."""
    t = cfg[PRODUCTS[product][0]]
    ...
    else:  # CO: GEE pre-filters on qa; add the solar-zenith cut
        coll = coll.map(lambda img: img.updateMask(img.select("solar_zenith_angle").lte(t["solar_zenith_max"])))
...
def fetch(coll, n, cfg, product="no2"):
    band = cfg[PRODUCTS[product][0]]["band"]
...
def run(start: str, end: str, product: str = "no2"):
    cfg = load_config()
    out_dir = PRODUCTS[product][1]
    ...
    for m_start, m_end in month_starts(start, end, cfg["study"]["season_months"]):
        out = out_dir / f"{m_start[:7]}.npz"
        if out.exists():
            continue
        coll, n = month_images(m_start, m_end, cfg, corridor, n_total, product)
        if n == 0:
            ...
            continue
        no2, t, frac = fetch(coll, n, cfg, product)
...
if __name__ == "__main__":
    ...
    p.add_argument("--product", choices=list(PRODUCTS), default="no2")
    args = p.parse_args()
    init_ee()
    run(args.start, args.end, args.product)
